...
class Player:
    ...
    Attributes:
        cards (tuple): '가위', '바위', '보'를 담고 있는 클래스 속성
        __name (str): 선수 이름
        __energy (int): 선수의 에너지 레벨

    Methods:
       __init__(self, name: str, energy: int) -> None:
       name(self) -> str:
       energy(self) -> int:
       status(self) -> None:
       pick(self) -> str:
    ...
    def pick(self):
        """가위, 바위, 보 중 임의로 하나를 선택해서 반환하는 메소드다.
...
def play(pick1, pick2):
    ...
    Args:
        pick1 (int): 첫 번째 선수가 낸 가위, 바위, 보 중 하나
        pick2 (int): 두 번째 선수가 낸 가위, 바위, 보 중 하나

    Returns:
        int: 0은 무승부, 1은 첫 번째 선수의 승리, 2는 두 번째 선수의 승리
    """
