...
# 과일 목록을 튜플로 만들어 변수 fruits에 할당한다.
fruits = '사과', '딸기', '블루베리', '바나나', '포도'

# 검색할 과일을 입력받아서 변수 target에 할당한다.
target = input('과일 이름을 입력하세요...: ')

# 시작 번호를 1로 설정해서 튜플 전체를 순회한다.
for index, fruit in enumerate(fruits, start=1):
    if fruit == target:  # 검색하는 과일이 튜플에 있다면
        # 튜플에서의 위치를 출력한 후 for문을 빠져 나간다.
        print(f'과일 목록의 {index}번째에 존재합니다.')
        break
else:                                # 튜플을 모두 순회한 후 검색하는 과일이 없다면
    print('과일 목록에 존재하지 않습니다.')  # 목록에 없다고 출력하고 for문을 종료한다.
