...
def wines(dataset, alcohol, flavonoid):
    """와인 리스트에서 지정한 알코올 도수보다는 낮고, 플라보노이드의 수치는 높은 와인의 개수를 반환하는
    함수다.

    Args:
        dataset (list): 전처리한 wine quality 데이터 세트
        alcohol (int | float): 알코올 도수
        flavonoid (int | float): 플라보노이드 수치
...
if __name__ == '__main__':
    ...
    for i, j in test_data:
        print(f'테스트 전달인자: 알코올 도수 = {i}, 플라보노이드 수치 = {j}')
        print(f'추천 와인 수..: {wines(wine_data, i, j)}', '\n')
