...
def installment(total_price, payment_amount, interest_rate):
    """매달 특정 할부금을 납입한다는 가정 아래 물건 가격을 전부 갚기 위해 필요한 전체 기간을 월 단위로
    계산해서 출력하는 함수다.

    Args:
        total_price (int): 물건 가격
        payment_amount (int): 매달 납입할 할부금
...
while True:  # 대화형 모드에서 ex10_7을 import하면 바로 while문이 실행된다.
    msg_err = '오류 : 잘못된 숫자입니다!'                 # 잘못 입력했을 때 출력하는 오류 메시지다.

    msg_continue = '계속하시겠습니까? (1)예 (2)아니요 => '  # 프로그램을 계속 진행할지 묻는 메시지다.
